...
if ($order_id) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sender_id = $worker_id; // Worker is sending the message
        $receiver_id = $customer_id; // Customer is the receiver
        $status = 'sent';
        $sender_type = 'worker'; // Sender type is worker
    ...
    // Fetch chat messages for this order, sorted by message_time in descending order
                                        ...
                                        <div class="msg-body">
                                            <ul>
                                                <?php while ($chat_row = $chat_result->fetch_assoc()): ?>
                                                    <?php if ($chat_row['sender_type'] == 'worker'): ?>
                                                        <!-- Worker is the sender, show on right -->
                                                        <li class="repaly">
                                                            <p><?php echo htmlspecialchars($chat_row['message_content']); ?></p>

                                                        </li>
                                                    <?php elseif ($chat_row['sender_type'] == 'customer'): ?>
                                                        <!-- Customer is the sender, show on left -->
                                                        <li class="sender">
                                                            <p><?php echo htmlspecialchars($chat_row['message_content']); ?></p>
